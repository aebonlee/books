...
function getStatusBadge(status: string, locale: string): ReactNode {
  const config: Record<string, { label: Record<string, string>; color: string; icon: ReactNode }> = {
    pending: {
...
export default function CustomOrderPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        ko ? '제작 의뢰가 등록되었습니다' : 'Your request has been submitted',
        'success',
      );
      setShowForm(false);
      setForm({
      ...
      showToast(
        err instanceof Error ? err.message : ko ? '등록에 실패했습니다' : 'Failed to submit',
        'error',
      );
    } finally {
      setSubmitting(false);
  ...
  const handleDelete = async (id: number) => {
    const msg = ko ? '이 의뢰를 삭제하시겠습니까?' : 'Are you sure you want to delete this request?';
    ...
    try {
      ...
      showToast(ko ? '삭제에 실패했습니다' : 'Failed to delete', 'error');
    }
  };

  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      <SEOHead title={ko ? '맞춤 제작의뢰' : 'Custom Request'} />

      <div className="mb-8 flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
            {ko ? '맞춤 제작의뢰' : 'Custom Request'}
          </h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="mt-2 text-gray-400">
            {ko ? '첫 번째 맞춤 제작을 의뢰해보세요' : 'Be the first to submit a custom request'}
          </p>
        </div>
      ) : (
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                      ...
                      <span>{getTypeLabel(req.request_type, language)}</span>
                      <span>{new Date(req.created_at).toLocaleDateString(ko ? 'ko-KR' : 'en-US')}</span>
                    </div>
                  </div>
                  {isExpanded
                  ...
                  <div className="border-t border-gray-100 dark:border-gray-700 p-4">
                    <div className="space-y-3 text-sm">
                      ...
                      <div className="flex flex-wrap gap-4 text-gray-500 dark:text-gray-400">
                        ...
                        {req.deadline && (
                          <span>{ko ? '희망 납기일' : 'Deadline'}: {req.deadline}</span>
                        )}
                      </div>
