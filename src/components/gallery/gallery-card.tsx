...
export function GalleryCard({ item, locale = 'ko', layout = 'portrait' }: GalleryCardProps) {
  if (layout === 'landscape') {
    return <LandscapeGalleryCard item={item} locale={locale} />;
  }

  return <PortraitGalleryCard item={item} locale={locale} />;
}

function PortraitGalleryCard({ item, locale }: { item: GalleryItem; locale: string }) {
...
function LandscapeGalleryCard({ item, locale }: { item: GalleryItem; locale: string }) {
  const { addItem, isInCart } = useCart();
  const inCart = isInCart(item.slug);
      ...
      <CardContent className="p-3">
        ...
        <div className="mt-1.5 flex items-center justify-between">
          <div className="flex items-center gap-2 truncate">
            {item.author_name && (
              <p className="truncate text-xs text-gray-500">{item.author_name}</p>
            )}
            {item.is_free ? (
