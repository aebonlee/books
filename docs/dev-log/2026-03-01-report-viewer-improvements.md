...
| 항목 | 변경 전 | 변경 후 |
|------|---------|---------|
| iframe 크기 | 풀 폭 + `paddingBottom: 56.25%` | `max-w-4xl` (896px) + `aspect-ratio: 16/9` |
| iframe 비율 | 16:9 (풀 폭이라 너무 큼) | 16:9 유지 (max-width로 적절한 크기) |
| 도움말 | 없음 | "조작법" 풍선 도움말 버튼 추가 |

> **참고**: 초기에 `paddingBottom: 45%`로 축소했으나 슬라이드 비율이 깨져 보이는 문제 발생.
> 16:9 비율을 유지하면서 `max-w-4xl`로 컨테이너 폭을 제한하는 방식으로 최종 해결.
